...
class SelfAttention(nn.Module):
    ...
    def __init__(self, data_dim, dim_q):
        ...
        self.fastattention = FastAttention(dim_heads=1, 
                                        nb_features=data_dim,
                                        no_projection=True)
        self._layers = []
    ...
    def forward(self, input_data):
        ...
        # print('queries', queries.shape.sum())
        # print('keys', keys.shape)
        # print('value', dot.shape)
        att = self.fastattention.forward(queries, keys, keys)
        # print( 'attention', att.shape)
        # print('squeeze', att.sum(dim=1))
        return att.squeeze().sum(dim=1)
        # scaled_dot = torch.div(dot, torch.sqrt(torch.tensor(k).float()))
        print(self.projection_matrix)
        attention_result = favor_attention(queries, keys, dot,
...
class VisionTaskSolution(BaseTorchSolution):
    ...
    def _get_output(self, inputs, update_filter):
        ...
        patch_importance = self._attention(flattened_patches)
        # print('patch importance shape: ', patch_importance.shape) 
        # patch_importance_matrix.shape = (n, n)
        # patch_importance_matrix = torch.softmax(
            # attention_matrix.squeeze(), dim=-1)

        # patch_importance_matrix = self._attention(flattened_patches)
